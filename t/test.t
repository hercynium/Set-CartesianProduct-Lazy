...
is($combo->last_idx, $combo->count - 1, "does last_idx = size - 1?");

my @tuple = $combo->get(-1);
my @expected = $combo->get($#nums);
is(@tuple, @expected, "testing weirdness of index = -1");
@tuple = $combo->get($size);
@expected = $combo->get(0);
is(@tuple, @expected, "what if index = number of elements in the product");
@tuple = $combo->get($size * -1);
is(@tuple, @expected, "index = -size_of_product");
@tuple = $combo->get(1024);
@expected = $combo->get(1024 % $size);
is(@tuple, @expected, "mod test theory");
...
is(@tuple, @expected, "index = -1 in Cartesian product A x B x C");
...
is(@tuple, @expected, "testing if it is the last index");
